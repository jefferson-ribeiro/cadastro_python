...
def cadastrar_produto():
    while True:
        ...
        # Salva os produtos após o cadastro
        salvar_produtos()  
...
def cadastrar_pedido():
    while True:
        ...
        # Salva os pedidos após o cadastro
        salvar_pedidos()  
...
def salvar_clientes():
    with open('.\console\ecommerce_app\dados\clientes.txt', 'w') as arquivo:
        for cliente in clientes:
            arquivo.write(f'{cliente.codigo},{cliente.nome},{cliente.telefone},{cliente.cpf},{cliente.email}\n')

def salvar_produtos():
    with open('.\console\ecommerce_app\dados\produtos.txt', 'w') as arquivo:
        for produto in produtos:
            arquivo.write(f'{produto.codigo},{produto.nome},{produto.descricao},{produto.valor}\n')

def salvar_pedidos():
    with open('.\console\ecommerce_app\dados\pedidos.txt', 'w') as arquivo:
        for pedido in pedidos:
            produtos_pedidos = ';'.join([f'{prod.codigo}:{qtd}' for prod, qtd in pedido.produtos])
            arquivo.write(f'{pedido.codigo},{pedido.cliente.codigo},{produtos_pedidos}\n')
